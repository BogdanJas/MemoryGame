...
namespace MemoryGame
{
    static class easy
    {
        ...
        public static int countofWords = 4;
        ...
        public static void Game(string pathToSaveInformation)
        {
            ...
            if (rightAnswers == countofWords)
            {
                ...
                try
                {
                    File.WriteAllText($@"{pathToSaveInformation}", $"{name}|{DateTime.UtcNow.ToShortDateString()}|{Math.Round((stop - start).TotalSeconds, 1)}|{chances}");
                }
                catch (Exception ex)
                {
                    Console.WriteLine(ex.Message);
...
class coordinateOfWord
{
    public string Word;
    public List<string> wordsOpened = new List<string>();
}
}
